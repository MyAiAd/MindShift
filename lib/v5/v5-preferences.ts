...
export interface V5VoicePreferences {
...
export interface V5Preferences extends V5VoicePreferences {
  interactionMode: InteractionMode;
}

// LocalStorage keys
const KEYS = {
  INTERACTION_MODE: 'v5_interaction_mode',
  SELECTED_VOICE: 'v5_selected_voice',
  PLAYBACK_SPEED: 'v5_playback_speed',
  VAD_SENSITIVITY: 'v5_vad_sensitivity',
  MIC_ENABLED: 'v5_mic_enabled',
  SPEAKER_ENABLED: 'v5_speaker_enabled',
  GUIDED_MODE: 'v5_guided_mode',
} as const;

// Event names for in-tab communication
export const V5_EVENTS = {
  INTERACTION_MODE_CHANGED: 'v5-interaction-mode-changed',
  VOICE_SETTINGS_CHANGED: 'v5-voice-settings-changed',
} as const;
...
export function setInteractionMode(mode: InteractionMode): void {
  ...
  const event = new CustomEvent(V5_EVENTS.INTERACTION_MODE_CHANGED, {
    detail: mode,
  });
 ...
 */
export function getVoicePreferences(): V5VoicePreferences {
 ...
 */
export function setVoicePreferences(prefs: Partial<V5VoicePreferences>): void {
  if (typeof window === 'undefined') return;
  ...
  // Emit event for same-tab listeners
  const event = new CustomEvent(V5_EVENTS.VOICE_SETTINGS_CHANGED, {
    detail: prefs,
  });
  window.dispatchEvent(event);
  ...
  if (prefs.selectedVoice !== undefined) {
    const legacyEvent = new CustomEvent('v5-voice-changed', {
 ...
 */
export function getAllPreferences(): V5Preferences {
  return {
 ...
 */
export function useV5Preferences() {
  if (typeof window === 'undefined') {
    return {
