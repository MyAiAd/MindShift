  ...
  static create(): TreatmentPhase {
    return {
      ...
      steps: [
...

        {
          ...
          scriptedResponse: (userInput, context) => {
            ...
            if (returnTo === 'identity_future_check' && !bridgeUsed) {
              ...
              console.log(`🔍 IDENTITY_DISSOLVE_STEP_A: Using FUTURE bridge phrase for '${identity}'`);
            } else if (returnTo === 'identity_scenario_check' && !bridgeUsed) {
              prefix = 'Imagine that scenario and feel yourself being';
              context.metadata.identityBridgePhraseUsed = true;
              console.log(`🔍 IDENTITY_DISSOLVE_STEP_A: Using SCENARIO bridge phrase for '${identity}'`);
            } else {
        ...
        // FUTURE PROJECTION PATHWAY - 6 steps for when identity persists in future scenarios
        {
