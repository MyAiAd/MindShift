/**
 * V6 Preferences Module
 * Single source of truth for V6 interaction modes and voice settings
 */

export type InteractionMode = 'orb_ptt' | 'listen_only' | 'text_first';

export interface V6VoicePreferences {
...
export interface V6Preferences extends V6VoicePreferences {
  interactionMode: InteractionMode;
}

// LocalStorage keys
const KEYS = {
  INTERACTION_MODE: 'v6_interaction_mode',
  SELECTED_VOICE: 'v6_selected_voice',
  PLAYBACK_SPEED: 'v6_playback_speed',
  VAD_SENSITIVITY: 'v6_vad_sensitivity',
  MIC_ENABLED: 'v6_mic_enabled',
  SPEAKER_ENABLED: 'v6_speaker_enabled',
  GUIDED_MODE: 'v6_guided_mode',
} as const;

// Event names for in-tab communication
export const V6_EVENTS = {
  INTERACTION_MODE_CHANGED: 'v6-interaction-mode-changed',
  VOICE_SETTINGS_CHANGED: 'v6-voice-settings-changed',
} as const;
...
export function setInteractionMode(mode: InteractionMode): void {
  ...
  const event = new CustomEvent(V6_EVENTS.INTERACTION_MODE_CHANGED, {
    detail: mode,
  });
...
/**
 * Get all V6 voice preferences
 */
export function getVoicePreferences(): V6VoicePreferences {
 ...
 */
export function setVoicePreferences(prefs: Partial<V6VoicePreferences>): void {
  if (typeof window === 'undefined') return;
  ...
  // Emit event for same-tab listeners
  const event = new CustomEvent(V6_EVENTS.VOICE_SETTINGS_CHANGED, {
    detail: prefs,
  });
  window.dispatchEvent(event);
  ...
  if (prefs.selectedVoice !== undefined) {
    const voiceChangedEvent = new CustomEvent('v6-voice-changed', {
...
/**
 * Get all V6 preferences
 */
export function getAllPreferences(): V6Preferences {
  return {
 ...
 */
export function useV6Preferences() {
  if (typeof window === 'undefined') {
    return {
