...
export async function POST(request: NextRequest) {
  try {
    console.log('Treatment V7 API: POST request received');
    
    let requestBody;
    try {
      requestBody = await request.json();
      console.log('Treatment V7 API: Request body parsed:', requestBody);
    } catch (parseError) {
      console.error('Treatment V7 API: JSON parsing error:', parseError);
    ...
    console.log('Treatment V7 API: Extracted parameters:', { sessionId, userInput, userId, action, undoToStep });

    // Validate required fields
    if (!sessionId || !userId) {
      console.log('Treatment V7 API: Missing required fields');
      return NextResponse.json(
    ...
    try {
      ...
      if (authError) {
        console.error('Treatment V7 API: Authentication error:', authError);
        // Continue execution for development/testing but log the issue
        console.warn('Treatment V7 API: Continuing without strict auth verification for compatibility');
      }

      if (user && user.id !== userId) {
        console.error('Treatment V7 API: User ID mismatch:', { requestUserId: userId, authUserId: user.id });
        return NextResponse.json(
      ...
      console.log('Treatment V7 API: Authentication check completed', { 
        hasUser: !!user, 
      ...
      console.error('Treatment V7 API: Auth check failed, continuing anyway:', authCheckError);
      // Continue execution - this handles cases where server-side auth isn't working
    }

    console.log('Treatment V7 API: Processing action:', action);
    switch (action) {
      case 'start':
    ...
    console.error('Treatment V7 API error:', error);
    console.error('Treatment V7 API error stack:', error instanceof Error ? error.stack : 'No stack trace');
    console.error('Treatment V7 API error type:', typeof error);
    console.error('Treatment V7 API error constructor:', error?.constructor?.name);
    return NextResponse.json(
      { 
...
async function handleStartSession(sessionId: string, userId: string) {
  ...
  try {
    console.log('Treatment V7 API: Starting session:', { sessionId, userId });
    ...
    console.log('Treatment V7 API: State machine result:', result);
    ...
    console.log(`🚀 V7 PERFORMANCE: Cache hit rate: ${perfMetrics.cacheHitRate.toFixed(1)}%, Preloaded responses: ${perfMetrics.preloadedResponsesUsed}`);

    return NextResponse.json(finalResponse);
  } catch (error) {
    console.error('V7 Start session error:', error);
    return NextResponse.json(
      { error: 'Failed to start V7 session', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
async function handleContinueSession(sessionId: string, userInput: string, userId: string) {
  ...
  try {
    console.log('Treatment V7 API [v7-routing-fix]: Continuing session:', { sessionId, userId, userInput: userInput.substring(0, 50) + '...' });
    
    console.log('Treatment V7 API: About to call processUserInput...');
    // Process with state machine first (95% of cases)
    let result;
    try {
      result = await treatmentMachine.processUserInput(sessionId, userInput, { userId });
      console.log('Treatment V7 API: State machine continue result:', result);
    } catch (stateMachineError) {
      console.error('Treatment V7 API: State machine error:', stateMachineError);
      return NextResponse.json({
        error: 'V7 state machine processing failed',
    ...
    console.log('Treatment V7 API: Creating final response object...');
    ...
    console.log('Treatment V7 API: Checking if can continue...', { canContinue: result.canContinue, hasScriptedResponse: !!result.scriptedResponse });
    if (result.canContinue && result.scriptedResponse) {
      console.log('Treatment V7 API: Processing successful result...');
      
      // Handle special transition signals
      if (result.scriptedResponse === 'TRANSITION_TO_DIG_DEEPER') {
        console.log('Treatment V7 API: Detected transition signal, processing next step immediately');
        ...
        if (nextResult.canContinue && nextResult.scriptedResponse) {
          ...
          console.log('Treatment V7 API: Using next step result:', result);
        }
      }
      ...
      // V7 Note: Linguistic processing will be handled by V7 state machine internally
      ...
      console.log('Treatment V7 API: Final response construction', {
        nextStep: result.nextStep,
    ...
    console.error('V7 Continue session error:', error);
    return NextResponse.json(
      { error: 'Failed to process V7 input', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
async function handleResumeSession(sessionId: string, userId: string) {
  try {
    console.log('Treatment V7 API: Resuming session (explicit resume requested):', { sessionId, userId });
    ...
    console.log('Treatment V7 API: Context loaded:', { 
    ...
    if (error || !session) {
      ...
      console.log('Treatment V7 API: No existing session found, starting new session');
      return await handleStartSession(sessionId, userId);
    }
    ...
    if (interactionsError) {
      console.error('Treatment V7 API: Error loading interactions:', interactionsError);
    }
    ...
    console.error('V7 Resume session error:', error);
    // Fallback to starting a new session if resume fails
    console.log('Treatment V7 API: Resume failed, falling back to new session');
    return await handleStartSession(sessionId, userId);
  }
}
...
// are identical to V2 for now and will be updated as V7 features are implemented.
...
async function handleAIAssistance(
  ...
  try {
    ...
    console.log(`V7 AI assistance used for session ${sessionId}:`, {
  ...
  try {
    ...
    if (validationResult.needsCorrection) {
      ...
      if (validationType === 'general_emotion') {
        ...
        console.log(`🔍 V7_VALIDATION_CORRECTION: Storing originalEmotion="${emotion}" for follow-up`);
      }
      ...
      if (validationType === 'incomplete_emotion_context') {
        ...
        console.log(`🔍 V7_VALIDATION_CORRECTION: Setting showEmotionConfirmation=true for Yes/No buttons`);
      }
      ...
      console.log(`🔍 V7_VALIDATION_CORRECTION: Saved context with metadata:`, treatmentContext.metadata);
      ...
      console.log(`🔍 V7_VALIDATION_PASSED: Storing metadata from context:`, treatmentContext.metadata);
      ...
      if (validationType === 'incomplete_emotion_context') {
        if (userInput.toLowerCase() === 'yes' || userInput.toLowerCase() === 'y') {
          ...
          console.log(`🔍 V7_EMOTION_CONFIRMATION: User confirmed, constructing full problem statement: "${fullProblemStatement}"`);
          ...
          console.log(`🔍 V7_EMOTION_CONFIRMATION: User said no, asking for restatement`);
      ...
      if (treatmentContext.currentStep === 'mind_shifting_explanation' && treatmentContext.metadata.selectedMethod) {
        ...
        console.log(`🔍 V7_VALIDATION: Storing corrected problem statement: "${userInput}" for method: ${treatmentContext.metadata.selectedMethod}`);
    ...
    console.error('V7 AI validation error:', error);
...
async function handleGetStatus(sessionId: string, userId: string) {
  try {
    ...
    console.error('V7 Get status error:', error);
    return NextResponse.json(
      { error: 'Failed to get V7 session status' },
      { status: 500 }
    );
  }
}
...
async function saveSessionToDatabase(
  ...
  try {
    console.log('Treatment V7 API: Saving session to database:', { sessionId, userId });
    ...
    if (profileError) {
      console.error('Treatment V7 API: Profile fetch error:', profileError);
      // Continue without tenant_id for super admins
    }
    
    console.log('Treatment V7 API: User profile:', profile);
    ...
    const sessionData = {
      ...
      total_ai_cost: 0.00,
      total_ai_tokens: 0,
      // V7 specific metadata
      treatment_version: 'v7'
    };
    
    console.log('Treatment V7 API: Inserting session data:', sessionData);
    ...
    if (error) {
      console.error('Treatment V7 API: Database insert error:', error);
      // Don't fail the request if database save fails - the session can still work
      console.warn('Treatment V7 API: Continuing without database save');
    } else {
      console.log('Treatment V7 API: Session saved successfully');
    }
  } catch (error) {
    console.error('V7 Database save error:', error);
  ...
  try {
    ...
    (async () => {
      try {
        ...
        console.error('V7 Background stats update failed:', error, {
          sessionId,
    ...
    console.error('V7 Database interaction save error:', error);
    // Don't fail the request if database save fails
  }
}
...
async function updateSessionContextInDatabase(
  ...
  try {
    ...
    if (isCompleted) {
      ...
      console.log(`🎉 TREATMENT_COMPLETION_V7: Marking session ${sessionId} as completed`);
    }
    ...
    console.error('V7 Database context update error:', error);
...
function getPhaseForStep(stepId: string): string {
  // Map steps to their correct phases based on V7 state machine definition
  // Note: This will need to be updated based on actual V7 phase/step structure
...
async function handleUndo(sessionId: string, undoToStep: string, userId: string) {
  try {
    console.log('Treatment V7 API: Handling undo to step:', undoToStep, 'for session:', sessionId);
    ...
    try {
      ...
      console.log('Treatment V7 API: Context retrieved successfully');
    } catch (contextError) {
      console.error('Treatment V7 API: Failed to get context:', contextError);
      throw new Error(`Failed to get V7 treatment context: ${contextError instanceof Error ? contextError.message : 'Unknown context error'}`);
    }
    
    if (!context) {
      throw new Error('V7 treatment context is null or undefined');
    }
    
    console.log('V7 Current context before undo:', { 
      currentStep: context.currentStep, 
      currentPhase: context.currentPhase,
    ...
    try {
      ...
      console.log('Treatment V7 API: Phase steps retrieved for phase:', context.currentPhase);
    } catch (phaseError) {
      console.error('Treatment V7 API: Failed to get phase steps:', phaseError);
      // Clear all responses as a safe fallback
      await treatmentMachine.clearUserResponsesForUndo(sessionId, new Set());
      console.log('Treatment V7 API: Cleared all responses due to phase error');
    }
    ...
    if (phaseSteps && Array.isArray(phaseSteps)) {
      ...
      try {
        if (foundTargetStep) {
          ...
          console.log('Treatment V7 API: Cleared user responses after target step');
        } else {
          // If not found in current phase, clear all responses to be safe
          treatmentMachine.clearUserResponsesForUndo(sessionId, new Set());
          console.log('Treatment V7 API: Cleared all user responses - undoing to different phase');
        }
      } catch (clearError) {
        console.error('Treatment V7 API: Error clearing user responses:', clearError);
        // Continue - this isn't critical for the undo operation
      }
    } else {
      console.log('Treatment V7 API: No phase steps found, clearing all responses');
      try {
        treatmentMachine.clearUserResponsesForUndo(sessionId, new Set());
      } catch (clearError) {
        console.error('Treatment V7 API: Error clearing all responses:', clearError);
        // Continue - this isn't critical for the undo operation
      }
    }
    ...
    console.log('Treatment V7 API: Target step belongs to phase:', targetPhase);
    ...
    try {
      ...
      console.log('Treatment V7 API: Context updated successfully');
    } catch (updateError) {
      console.error('Treatment V7 API: Error updating context:', updateError);
      throw new Error(`Failed to update V7 context: ${updateError instanceof Error ? updateError.message : 'Unknown update error'}`);
    }
    ...
    if (goalRelatedSteps.includes(undoToStep)) {
      try {
        ...
        console.error('Treatment V7 API: Error restoring goal metadata:', restoreError);
    ...
    try {
      ...
      console.log('V7 Context after undo:', { 
      ...
      console.error('Treatment V7 API: Error getting updated context:', contextError);
    ...
    console.error('Treatment V7 API: Undo error:', error);
    return NextResponse.json(
      { error: 'V7 undo failed', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
} 