  ...
  try {
    ...
    if (result.canContinue && result.scriptedResponse) {
      ...
      // NEW: Handle AI validation requests
              const validationType = result.reason.split(':')[1] as 'problem_vs_goal' | 'problem_vs_question' | 'single_negative_experience' | 'goal_vs_problem' | 'goal_vs_question';
...
async function updateSessionContextInDatabase(
  ...
  try {
    ...
    if (isCompleted) {
      ...
      console.log(`🎉 TREATMENT_COMPLETION: Marking session ${sessionId} as completed`);
    }
