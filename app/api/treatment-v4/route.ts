...
import { TreatmentStateMachine } from '@/lib/v4/treatment-state-machine';
import { ProcessingResult } from '@/lib/v4/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('Treatment V4 API: Request body parsed:', requestBody);
    } catch (parseError) {
      console.error('Treatment V4 API: JSON parsing error:', parseError);
    ...
    if (!sessionId || !userId) {
      console.log('Treatment V4 API: Missing required fields');
      return NextResponse.json(
    ...
    try {
      ...
      if (authError) {
        ...
        console.warn('Treatment V4 API: Continuing without strict auth verification for compatibility');
      }

      if (user && user.id !== userId) {
        console.error('Treatment V4 API: User ID mismatch:', { requestUserId: userId, authUserId: user.id });
        return NextResponse.json(
      ...
      console.error('Treatment V4 API: Auth check failed, continuing anyway:', authCheckError);
      // Continue execution - this handles cases where server-side auth isn't working
    }

    console.log('Treatment V4 API: Processing action:', action);
    switch (action) {
      case 'start':
    ...
    console.error('Treatment V4 API error:', error);
    console.error('Treatment V4 API error stack:', error instanceof Error ? error.stack : 'No stack trace');
...
async function handleContinueSession(sessionId: string, userInput: string, userId: string) {
  ...
  try {
    ...
    try {
      ...
      console.log('Treatment V4 API: State machine continue result:', result);
    ...
    if (result.canContinue && result.scriptedResponse) {
      ...
      if (result.needsLinguisticProcessing) {
        ...
        if (shouldSkipAI) {
          ...
          if (linguisticResult.success) {
            ...
            console.log('Treatment V4 API: Linguistic processing successful');
          } else {
            console.log('Treatment V4 API: Linguistic processing failed, using scripted response');
          }
        }
      }
      ...
      console.log('Treatment V4 API: Final response construction', {
...
async function handleResumeSession(sessionId: string, userId: string) {
  try {
    ...
    if (error || !session) {
      ...
      console.log('Treatment V4 API: No existing session found, starting new session');
      return await handleStartSession(sessionId, userId);
    }
    ...
    if (interactionsError) {
      console.error('Treatment V4 API: Error loading interactions:', interactionsError);
    }
...
async function handleUndo(sessionId: string, undoToStep: string, userId: string) {
  try {
    ...
    try {
      ...
      console.log('Treatment V4 API: Phase steps retrieved for phase:', context.currentPhase);
    } catch (phaseError) {
      console.error('Treatment V4 API: Failed to get phase steps:', phaseError);
    ...
    if (phaseSteps && Array.isArray(phaseSteps)) {
      ...
      try {
        if (foundTargetStep) {
          ...
          console.log('Treatment V4 API: Cleared user responses after target step');
        } else {
          // If not found in current phase, clear all responses to be safe
          treatmentMachine.clearUserResponsesForUndo(sessionId, new Set());
          console.log('Treatment V4 API: Cleared all user responses - undoing to different phase');
        }
      } catch (clearError) {
        console.error('Treatment V4 API: Error clearing user responses:', clearError);
        // Continue - this isn't critical for the undo operation
      }
    } else {
      console.log('Treatment V4 API: No phase steps found, clearing all responses');
      try {
        treatmentMachine.clearUserResponsesForUndo(sessionId, new Set());
      } catch (clearError) {
        console.error('Treatment V4 API: Error clearing all responses:', clearError);
        // Continue - this isn't critical for the undo operation
      }
    }
    ...
    try {
      ...
      console.error('Treatment V4 API: Error getting updated context:', contextError);
