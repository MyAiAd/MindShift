-- Add stats_cleared_at column to profiles table for statistics reset functionality
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS stats_cleared_at TIMESTAMP WITH TIME ZONE DEFAULT NULL;

-- Add index for performance when filtering by stats_cleared_at
CREATE INDEX IF NOT EXISTS idx_profiles_stats_cleared_at ON profiles(stats_cleared_at);

-- Update the get_session_stats function to respect stats_cleared_at
