...
interface IdentityShiftingStep {
  id: string;
  title: string;
...
// EXACT Identity Shifting steps from the working treatment state machine
const IDENTITY_SHIFTING_STEPS: IdentityShiftingStep[] = [
  {
    id: 'problem_capture',
  ...
  },
  {
    ...
    title: 'Identity Shifting Introduction',
  ...
  },
  {
    ...
    title: 'Identity Dissolve Step A',
    script: (userInput: string, context: any) => {
  ...
  },
  {
    ...
    title: 'Identity Dissolve Step B',
  ...
  },
  {
    ...
    title: 'Identity Dissolve Step C',
    script: (userInput: string, context: any) => {
      const identity = context?.userResponses?.['identity_shifting_intro'] || 'that identity';
  ...
  },
  {
    ...
    script: 'Thank you for participating in this Identity Shifting demo. The treatment process has been completed.',
    expectedResponseType: 'open'
  }
];

export default function IdentityShiftingDemo() {
  ...
  const currentStep = IDENTITY_SHIFTING_STEPS[currentStepIndex];
  ...
  const getScriptedResponse = (step: IdentityShiftingStep, userInput: string = '', context?: any): string => {
  ...
  // Validation logic for Identity Shifting (same as Problem Shifting)
  ...
  const speakText = async (text: string) => {
    ...
    try {
      ...
      // Try browser TTS first for speed
  ...
  const processUserTranscript = async (transcript: string) => {
    const currentStepFromRef = IDENTITY_SHIFTING_STEPS[currentStepIndexRef.current];
    ...
    if (currentIndex < IDENTITY_SHIFTING_STEPS.length - 1) {
      const nextIndex = currentIndex + 1;
      setCurrentStepIndex(nextIndex);
      
      const nextStep = IDENTITY_SHIFTING_STEPS[nextIndex];
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Identity Shifting Demo</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Voice-guided Identity Shifting treatment with manual speech control
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-md">
          <div className="flex items-center justify-between mb-2">
            <h5 className="font-medium text-indigo-900 dark:text-indigo-200">
              Step {currentStepIndex + 1} of {IDENTITY_SHIFTING_STEPS.length}: {currentStep.title}
            </h5>
          ...
          <div className="w-full bg-indigo-200 dark:bg-indigo-800 rounded-full h-2">
            <div 
              ...
              style={{ width: `${((currentStepIndex + 1) / IDENTITY_SHIFTING_STEPS.length) * 100}%` }}
            ></div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        >
          ...
          <span>Start Identity Shifting</span>
        </button>
        ...
        <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md flex items-center">
          ...
          <div className="flex-1">
            <span className="text-sm text-green-800 dark:text-green-200">
              🎙️ Identity Shifting demo active! 
      ...
      <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900/20 rounded-lg">
        ...
        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
          {IDENTITY_SHIFTING_STEPS.map((step, index) => (
