...
interface TraumaShiftingStep {
  id: string;
  title: string;
  script: string | ((userInput: string, context: any) => string);
  expectedResponseType: 'experience' | 'feeling' | 'open' | 'yesno';
  nextStep?: string;
}

// Simplified Trauma Shifting steps
const TRAUMA_SHIFTING_STEPS: TraumaShiftingStep[] = [
  {
    id: 'experience_capture',
    title: 'Negative Experience Statement',
    script: 'Mind Shifting is not like counselling, therapy or life coaching. The Mind Shifting methods are verbal guided processes that we apply to problems, goals, or negative experiences in order to clear them. Please tell me about a negative experience you want to work on in a few words.',
    expectedResponseType: 'experience',
    nextStep: 'trauma_shifting_intro'
  },
  {
    id: 'trauma_shifting_intro',
    title: 'Trauma Shifting Introduction',
    script: (userInput: string, context: any) => {
      const experienceStatement = context?.experienceStatement || 'the experience';
      return `Please close your eyes and keep them closed throughout the process. Feel the negative experience '${experienceStatement}'... what kind of person are you being when you're experiencing this?`;
    },
  ...
  },
  {
    ...
    title: 'Trauma Dissolve Step A',
    script: (userInput: string, context: any) => {
      const identity = userInput || 'that identity';
  ...
  },
  {
    ...
    title: 'Trauma Dissolve Step C',
    script: (userInput: string, context: any) => {
      const identity = context?.userResponses?.['trauma_shifting_intro'] || 'that identity';
  ...
  },
  {
    ...
    script: 'Thank you for participating in this Trauma Shifting demo. The treatment process has been completed.',
    expectedResponseType: 'open'
  }
];

export default function TraumaShiftingDemo() {
  ...
  const currentStep = TRAUMA_SHIFTING_STEPS[currentStepIndex];
  ...
  const getScriptedResponse = (step: TraumaShiftingStep, userInput: string = '', context?: any): string => {
  ...
  // Validation logic for Trauma Shifting (negative experience validation)
  const validateUserInput = (transcript: string, stepId: string): { isValid: boolean; error?: string } => {
    const trimmed = transcript.trim();
    const lowerInput = trimmed.toLowerCase();
    const words = trimmed.split(/\s+/).length;

    if (stepId === 'experience_capture') {
      // Check for multiple events
      const multipleEventIndicators = ['always', 'often', 'repeatedly', 'throughout', 'as a child', 'growing up', 'every time', 'whenever'];
      const hasMultipleEvents = multipleEventIndicators.some(indicator => lowerInput.includes(indicator));
      
      if (hasMultipleEvents) {
        return { isValid: false, error: 'It is important that we only work on one memory of a single event at a time, so please recall a significant event and tell me what the event was in a few words.' };
      }

      if (words < 2) {
        return { isValid: false, error: 'Please tell me a bit more about the negative experience you want to work on.' };
      }
    }
  ...
  const processUserTranscript = async (transcript: string) => {
    const currentStepFromRef = TRAUMA_SHIFTING_STEPS[currentStepIndexRef.current];
    ...
    if (currentStepFromRef.id === 'experience_capture') {
      newContext.experienceStatement = transcript;
    }
    ...
    if (currentIndex < TRAUMA_SHIFTING_STEPS.length - 1) {
      const nextIndex = currentIndex + 1;
      setCurrentStepIndex(nextIndex);
      
      const nextStep = TRAUMA_SHIFTING_STEPS[nextIndex];
  ...
  const startDemo = async () => {
    ...
    setSessionContext({ experienceStatement: '', userResponses: {} });
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Trauma Shifting Demo</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Voice-guided Trauma Shifting treatment with manual speech control
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-md">
          <div className="flex items-center justify-between mb-2">
            <h5 className="font-medium text-indigo-900 dark:text-indigo-200">
              Step {currentStepIndex + 1} of {TRAUMA_SHIFTING_STEPS.length}: {currentStep.title}
            </h5>
          ...
          <div className="w-full bg-indigo-200 dark:bg-indigo-800 rounded-full h-2">
            <div 
              ...
              style={{ width: `${((currentStepIndex + 1) / TRAUMA_SHIFTING_STEPS.length) * 100}%` }}
            ></div>
          </div>
        </div>
      )}
      ...
      {/* Working On Display */}
      {sessionContext.experienceStatement && (
        <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
          <h5 className="font-medium text-green-900 dark:text-green-200 mb-1">Working On:</h5>
          <p className="text-sm text-green-700 dark:text-green-300">Experience: "{sessionContext.experienceStatement}"</p>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        >
          ...
          <span>Start Trauma Shifting</span>
        </button>
      ...
      <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-900/20 rounded-lg">
        <h6 className="font-medium text-gray-900 dark:text-white mb-2">Trauma Shifting Steps:</h6>
        <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
          {TRAUMA_SHIFTING_STEPS.map((step, index) => (
