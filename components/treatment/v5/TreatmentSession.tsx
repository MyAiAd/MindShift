...
import { V4_STATIC_AUDIO_TEXTS } from '@/lib/v5/static-audio-texts';
// V4 preferences for interaction modes
import { getInteractionMode, getVoicePreferences, shouldShowOrb, shouldShowTextFirst, isListenOnlyMode, InteractionMode, V5_EVENTS } from '@/lib/v5/v5-preferences';
...
export default function TreatmentSession({
  ...
  const [isMicEnabled, setIsMicEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v5_mic_enabled') === 'true';
    }
    return false;
  });
  ...
  const [isSpeakerEnabled, setIsSpeakerEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v5_speaker_enabled') === 'true';
    }
    return false;
  });
  ...
  const [isNaturalVoiceEnabled, setIsNaturalVoiceEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v5_natural_voice') === 'true';
    }
    return false;
  });
  ...
  const checkMicPermission = useCallback(async (): Promise<'granted' | 'denied' | 'prompt'> => {
    ...
    const cached = localStorage.getItem('v5_mic_permission');
  ...
  const requestMicPermission = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem('v5_mic_permission', 'granted');
      setMicPermission('granted');
      return true;
    } catch (e) {
      localStorage.setItem('v5_mic_permission', 'denied');
      setMicPermission('denied');
      return false;
    }
  }, [checkMicPermission]);
  ...
  const toggleMic = useCallback(async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('v5_mic_enabled', String(newState));
    }
  ...
  const toggleSpeaker = useCallback(() => {
    ...
    // Sticky Settings - persist to localStorage
    if (typeof window !== 'undefined') {
      localStorage.setItem('v5_speaker_enabled', String(newState));
    }
  ...
  const toggleNaturalVoice = () => {
    ...
    // Sticky Settings
    if (typeof window !== 'undefined') {
      localStorage.setItem('v5_natural_voice', String(newState));
    }
  ...
  useEffect(() => {
    const handleModeChange = (e: Event) => {
      ...
      if (customEvent.detail === 'orb_ptt') {
        ...
        setIsSpeakerEnabled(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('v5_mic_enabled', 'false');
          localStorage.setItem('v5_speaker_enabled', 'true');
    ...
    window.addEventListener(V5_EVENTS.INTERACTION_MODE_CHANGED, handleModeChange);
    window.addEventListener('resize', handleResize);

    return () => {
      window.removeEventListener(V5_EVENTS.INTERACTION_MODE_CHANGED, handleModeChange);
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/treatment-v5', {
        method: 'POST',
        headers: {
