 ...
 * 2. Files are stored in public/audio/v5/static/[voice-name]/
...
// Manifest will be loaded from public/audio/v5/static/[voice]/manifest.json
...
interface V5AudioPreloaderProps {
  voice?: string; // 'heart' | 'michael' etc.
}

export default function V5AudioPreloader({ voice = 'heart' }: V5AudioPreloaderProps) {
  useEffect(() => {
    ...
    const preloadStaticAudio = async () => {
      ...
      console.log(`🎵 V5: Starting audio preload for voice "${voice}" from static files...`);

      try {
        // Load the manifest for the selected voice
        const manifestResponse = await fetch(`/audio/v5/static/${voice}/manifest.json`);
        ...
        console.log(`✅ V5 Audio preload complete for "${voice}"!`);
