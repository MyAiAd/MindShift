...
// V6 static audio texts (for consistency with preloader)
import { V6_STATIC_AUDIO_TEXTS } from '@/lib/v6/static-audio-texts';
// V6 preferences for interaction modes
import {
  getInteractionMode,
  getVoicePreferences,
  shouldShowOrb,
...
export default function TreatmentSession({
  ...
  const [isDebugDrawerOpen, setIsDebugDrawerOpen] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v6_debug_drawer_open') === 'true';
    }
    return false;
  });
  const toggleDebugDrawer = useCallback(() => {
    setIsDebugDrawerOpen((prev) => {
      const next = !prev;
      if (typeof window !== 'undefined') {
        localStorage.setItem('v6_debug_drawer_open', String(next));
      }
      return next;
    });
  }, []);
  ...
  const [isMicEnabled, setIsMicEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v6_mic_enabled') === 'true';
    }
    return false;
  });
  ...
  const [isSpeakerEnabled, setIsSpeakerEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v6_speaker_enabled') === 'true';
    }
    return false;
  });
  ...
  const [isNaturalVoiceEnabled, setIsNaturalVoiceEnabled] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v6_natural_voice') === 'true';
    }
    return false;
  });
  ...
  const checkMicPermission = useCallback(async (): Promise<'granted' | 'denied' | 'prompt'> => {
    ...
    const cached = localStorage.getItem('v6_mic_permission');
  ...
  const requestMicPermission = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      localStorage.setItem('v6_mic_permission', 'granted');
      setMicPermission('granted');
      return true;
    } catch (e) {
      localStorage.setItem('v6_mic_permission', 'denied');
      setMicPermission('denied');
      return false;
    }
  }, [checkMicPermission]);
  ...
  const [playbackSpeed, setPlaybackSpeed] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('v6_playback_speed');
  ...
  const [selectedVoice, setSelectedVoice] = useState(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('v6_selected_voice') || 'heart';
    }
    return 'heart';
  });
  const [isGuidedMode, setIsGuidedMode] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('v6_guided_mode');
      return saved === 'true';
    }
    return false;
  });
  ...
  const [vadSensitivity, setVadSensitivity] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('v6_vad_sensitivity');
  ...
  const toggleMic = useCallback(async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_mic_enabled', String(newState));
    }
  ...
  const toggleSpeaker = useCallback(() => {
    ...
    // Sticky Settings - persist to localStorage
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_speaker_enabled', String(newState));
    }
  ...
  const toggleNaturalVoice = () => {
    ...
    // Sticky Settings
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_natural_voice', String(newState));
    }
  ...
  const handleSpeedChange = (newSpeed: number) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_playback_speed', String(newSpeed));
    }
  };
  ...
  const handleVoiceChange = (voiceId: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_selected_voice', voiceId);
  ...
  const handleVadSensitivityChange = (newSensitivity: number) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('v6_vad_sensitivity', String(newSensitivity));
    }
      ...
      requestMicPermission().then(granted => {
        if (granted) {
          ...
          if (!isMicEnabled) {
            ...
            localStorage.setItem('v6_mic_enabled', 'true');
          }
        } else {
  ...
  useEffect(() => {
    const handleModeChange = (e: Event) => {
      ...
      if (customEvent.detail === 'orb_ptt') {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('v6_mic_enabled', 'true');
          localStorage.setItem('v6_speaker_enabled', 'true');
          localStorage.setItem('v6_guided_mode', 'true');
        }
        ...
        setIsSpeakerEnabled(true);
        if (typeof window !== 'undefined') {
          localStorage.setItem('v6_mic_enabled', 'false');
          localStorage.setItem('v6_speaker_enabled', 'true');
          localStorage.setItem('v6_guided_mode', 'false');
        }
    ...
    window.addEventListener(V6_EVENTS.INTERACTION_MODE_CHANGED, handleModeChange);
    window.addEventListener('resize', handleResize);

    return () => {
      window.removeEventListener(V6_EVENTS.INTERACTION_MODE_CHANGED, handleModeChange);
  ...
  const formatMethodName = (methodName: string) => {
    if (!methodName) return 'Mind Shifting V6';
  ...
  const startSession = async () => {
    ...
    try {
      ...
      const instantMessage: TreatmentMessage = {
        id: 'system-init',
        content: V6_STATIC_AUDIO_TEXTS.INITIAL_WELCOME,
        isUser: false,
        timestamp: new Date(),
        version: 'v6'
      };
      ...
      const response = await fetch('/api/treatment-v6', {
        method: 'POST',
        headers: {
      ...
      setHasError(true);
      setErrorMessage(error instanceof Error ? error.message : 'Unknown V6 error');
      onError?.(error instanceof Error ? error.message : 'Unknown V6 error');
  ...
  const sendMessage = async (content: string, isAutoAdvance = false) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        throw new Error(data.error || 'Failed to process V6 message');
      }
    } catch (error) {
      ...
      const errorMessage: TreatmentMessage = {
        id: `error-${Date.now()}`,
        content: `V6 Error: ${error instanceof Error ? error.message : 'Unknown error'}. Please try again.`,
        isUser: false,
        timestamp: new Date(),
        version: 'v6'
      };
