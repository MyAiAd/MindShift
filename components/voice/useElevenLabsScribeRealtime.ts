...
export function useElevenLabsScribeRealtime({
  ...
  const connectWebSocket = useCallback(async () => {
    ...
    // VAD-mode tuning: shrink the silence window from the 1.5s default to 0.5s
    // (aggressive profile) so Scribe emits `committed_transcript` ~1s sooner
    // once the patient stops talking. `min_silence_duration_ms=200` makes sure
    // normal in-word breaths don't trigger a premature commit. Only meaningful
    // when commit_strategy=vad — the params are ignored in manual mode.
    // Tradeoff: at 0.5s, mid-sentence thinking pauses can split an utterance
    // into two committed transcripts; the per-provider accumulator window in
    // TreatmentSession.tsx (TRANSCRIPT_FLUSH_DELAY_MS_SCRIBE) catches them.
