  ...
  return (
    <div className={cn('animate-pulse', className)} role="status" aria-live="polite" aria-label="Loading content">
      {variant === 'card' && items.map((i) => (
        <div key={i} className="bg-card rounded-lg shadow-sm border border-border p-4 sm:p-6 mb-4">
      ...
      {variant === 'profile' && (
        <div className="bg-card rounded-lg shadow-sm border border-border p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
            <div className="w-24 h-24 bg-secondary rounded-full flex-shrink-0" />
            <div className="flex-1 space-y-4 w-full">
              <div className="space-y-2">
                <div className="h-6 bg-secondary rounded w-48 mx-auto sm:mx-0" />
                <div className="h-4 bg-secondary rounded w-32 mx-auto sm:mx-0" />
              </div>
              <div className="space-y-2">
                <div className="h-4 bg-secondary rounded w-full" />
                <div className="h-4 bg-secondary rounded w-5/6" />
      ...
      {variant === 'dashboard' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-card rounded-lg shadow-sm border border-border p-4 sm:p-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="h-4 bg-secondary rounded w-20" />
                  <div className="w-5 h-5 bg-secondary rounded" />
                </div>
                <div className="h-8 bg-secondary rounded w-16 mb-1" />
...
export function SkeletonLine({ className, width = 'full' }: { className?: string; width?: 'full' | 'half' | 'quarter' | 'three-quarter' }) {
  ...
  return (
    <div className={cn('h-4 bg-secondary rounded animate-pulse', widthClasses[width], className)} />
  );
}
...
export function SkeletonCircle({ className, size = 'md' }: { className?: string; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className={cn('bg-secondary rounded-full animate-pulse', sizeClasses[size], className)} />
  );
}

export function SkeletonButton({ className }: { className?: string }) {
  return (
    <div className={cn('h-10 bg-secondary rounded-lg animate-pulse w-24', className)} />
  );
}
